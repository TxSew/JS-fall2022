...
// sử lý đăng ký
const xuLyRegister = () => {
  const elUsername = document.getElementById("username").value;
  const elPassword = document.getElementById("password").value;
  ...
  elButtonLogin.addEventListener("click", async (event) => {
    ...
    if (errors.length > 0) {
      // show error message
      elErros.innerHTML = errors.join("</br>");
      console.log(elEmail, elPassword, elUsername);
      return;
    } else {
      // dang nhap thanh cong
